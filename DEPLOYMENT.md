...
8. **Email alert delivery.** Create a real Resend account, verify a sending domain, set `RESEND_API_KEY` and `RESEND_FROM_EMAIL` (e.g. `EdgeHub <alerts@yourdomain.com>`) - no rebuild needed, read at runtime only. Without `RESEND_FROM_EMAIL`, sends use Resend's own sandbox address (`onboarding@resend.dev`), which works with no domain verification but is meant for testing, not real delivery. See README.md's "Email alert delivery" for what's verified vs. not (no Resend account exists in this environment to test actual delivery against - the integration was verified against Resend's real API over the network with a fake key, confirming the round trip works, just not that a real email lands anywhere).
